...
public class PartidaXadrez {
	...
	private PecaXadrez rei(Color color) {
		List<Piece> list = pecasNoTabuleiro.stream().filter(x -> ((PecaXadrez)x).getColor() == color).collect(Collectors.toList());
		for(Piece p : list) {
			if(p instanceof Rei) {
				return (PecaXadrez)p;
			}
	...
	private boolean testarCheque(Color color) {
		...
		List<Piece> pecasOponentes = pecasNoTabuleiro.stream().filter(x -> ((PecaXadrez)x).getColor() == oponente(color)).collect(Collectors.toList());
